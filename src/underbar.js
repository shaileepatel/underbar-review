(function() {
  ...
  _.last = function(array, n) {
    ...
    if (n === undefined) {
      ...
      return copy.slice(array.length - n);
    }
  };
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (iterator !== undefined) {
      _.each(array, function(item, index) {
        let truthy = true;
        _.each(result, function(elem, i) {
          if (iterator(elem) === iterator(item)) {
            truthy = false;
          }
        });
        if (truthy) { result.push(item); }
      });
    }
    return result;
  };
  ...
  _.shuffle = function(array) {
    ...
    _.each(array, function(item, i) {
      ...
      if (placement[rand] === undefined) {
        placement[rand] = item;
      } else {
        while (placement[rand] !== undefined) {
    ...
    let result = [];
    _.each(placement, function(value, key) {
      _.each(array, function(item, i) {
        if (key === i.toString()) {
          result.push(value);
        }
